    ...
    return (
        <nav className={cn(
            ...
            <div className="h-full px-4 md:px-8 flex items-center justify-between gap-4 max-w-[1920px] mx-auto relative">
                ...
                <div className={cn(
                    "absolute inset-0 bg-background/95 backdrop-blur-xl z-50 flex items-center px-4 gap-4 md:hidden transition-all duration-300",
                    isSearchOpen ? "opacity-100 pointer-events-auto" : "opacity-0 pointer-events-none"
                )}>
                    <button
                        onClick={() => setIsSearchOpen(false)}
                        className="p-2 -ml-2 hover:bg-white/5 rounded-xl text-text-secondary"
                    >
                        <X size={20} />
                    </button>
                    <form onSubmit={handleSearch} className="flex-1 relative">
                        <input
                            ...
                            className="w-full h-10 pl-4 pr-10 bg-panel border-2 border-border rounded-xl text-sm outline-none focus:border-accent-blue"
                        />
                        <button type="submit" className="absolute right-3 top-1/2 -translate-y-1/2 text-accent-blue">
                            <Search size={18} />
                        </button>
                    </form>
                </div>
