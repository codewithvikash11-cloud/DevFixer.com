...
export default function ErrorsList({ initialPosts = [] }) {
    ...
    // Given the previous code just fetched /api/posts and filtered client side, we will stick to that pattern but with WP data.

    // However, WP API is paginated. The Server Component fetches page 1.
    // If we want "pure" client side filtering as before, we'd need ALL posts.
    // The previous code had: fetch('/api/posts?status=published').
    // Let's assume initialPosts contains a reasonable number for now, or we implement basic filtering on what we have.
    // To strictly follow "Keep same filters UI", client-side filtering is smoothest if we have the data.
    // Let's treat initialPosts as the full list or the list to filter for now.

    // Adapt WP data fields to the UI if needed, although the Server Component will likely map them.
    // Lets assume the Server Component passes mapped data.
    ...
    const filteredPosts = useMemo(() => {
        ...
        if (searchQuery) {
            ...
            result = result.filter(p =>
                ...
                (p.language && p.language.toLowerCase().includes(lowerQ))
            );
        }
        ...
        // Sorting
        if (sortBy === 'popular') {
            // WP doesn't strictly have "likes" unless we use custom fields, defaulting to 0 or mock
                                ...
                                <div className="flex items-start justify-between gap-4">
                                    <div className="flex-1 min-w-0">
                                        <div className="flex items-center gap-2 mb-2">
                                            ...
                                            {post.language && <span className="w-1 h-1 rounded-full bg-text-tertiary" />}
                                        ...
                                        <p className="text-sm text-text-secondary line-clamp-2 md:line-clamp-1 mb-4" dangerouslySetInnerHTML={{ __html: post.description }} />

                                        <div className="flex items-center gap-4 text-xs text-text-tertiary">
                                            <div className="flex items-center gap-1">
                                                <TrendingUp size={12} />
                                                <span>{post.views || '100+'} views</span>
                                            </div>
                                            <div className="flex items-center gap-1">
                                                <Clock size={12} />
